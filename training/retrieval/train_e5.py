...
def load_data(path, task_description):
    if "json" in path:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        query = get_detailed_instruct(task_description, row["query"])
...
def main(args):
    ...
    task = "Với một truy vấn về luật Việt Nam, truy xuất các đoạn văn liên quan có chứa câu trả lời cho truy vấn đó."
    train_examples = load_data(args.train_data, task)
